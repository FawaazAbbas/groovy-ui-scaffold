  ...
  return (
    <SlideLayout className="bg-background">
      <div className="flex flex-col justify-center h-full px-[160px]">
        <span className="text-[18px] font-semibold tracking-widest uppercase text-electric-bright mb-6">Traction</span>
        ...
        <div className="grid grid-cols-2 gap-x-20 gap-y-14">
          {metrics.map((m) => (
            <div key={m.label}>
              <span className="text-[72px] font-bold text-electric-bright leading-none block">{m.value}</span>
              <span className="text-[26px] font-medium text-text-primary mt-3 block">{m.label}</span>
              <span className="text-[20px] text-text-secondary mt-1 block">{m.sub}</span>
            </div>
          ))}
        </div>
      </div>
